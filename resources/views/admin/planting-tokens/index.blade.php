...
<x-adminlte-card title="Daftar token akses" icon="bi bi-qr-code" theme="success">
    ...
    @forelse ($tokens as $token)
        ...
        </td><td>{{ $token->planting_records_count }} penanaman</td><td class="text-end text-nowrap"><a href="{{ route('admin.planting-tokens.show', $token) }}" class="btn btn-sm btn-outline-success"><i class="bi bi-qr-code"></i></a> <a href="{{ route('admin.planting-tokens.edit', $token) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a><form class="d-inline" method="POST" action="{{ route('admin.planting-tokens.destroy', $token) }}" onsubmit="return confirm('Hapus token ini?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></td></tr>
    @empty
